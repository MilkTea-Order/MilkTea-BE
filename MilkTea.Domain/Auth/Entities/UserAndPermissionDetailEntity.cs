namespace MilkTea.Domain.Auth.Entities;

public class UserAndPermissionDetailEntity
{
    public int UserID { get; private set; }
    public int PermissionDetailID { get; private set; }
    public int CreatedBy { get; private set; }
    public DateTime CreatedDate { get; private set; }

    private UserAndPermissionDetailEntity() { }

    public static UserAndPermissionDetailEntity Create(int userId, int permissionDetailId, int createdBy)
    {
        ArgumentOutOfRangeException.ThrowIfNegativeOrZero(userId);
        ArgumentOutOfRangeException.ThrowIfNegativeOrZero(permissionDetailId);
        ArgumentOutOfRangeException.ThrowIfNegativeOrZero(createdBy);

        return new UserAndPermissionDetailEntity
        {
            UserID = userId,
            PermissionDetailID = permissionDetailId,
            CreatedBy = createdBy,
            CreatedDate = DateTime.Now
        };
    }
}
