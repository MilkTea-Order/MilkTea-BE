...
public class UserRepository(AppDbContext context) : IUserRepository
{
    ...
    /// Uses AsNoTracking() for read-only queries.
    /// </remarks>
    public async Task<RefreshTokenEntity?> GetValidRefreshTokenByTokenAsync(string token, CancellationToken cancellationToken)
    {
    ...
    /// Uses AsNoTracking() for read-only queries.
    /// </remarks>
    public async Task<List<RefreshTokenEntity>> GetActiveRefreshTokensByUserIdAsync(int userId)
    {
        var now = DateTime.Now;
        return await _vContext.RefreshTokens
            .AsNoTracking()
